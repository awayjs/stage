...
export class ProgramGLES extends GLESAssetBase implements IProgram {
	...
	public upload(vertexProgram: ByteArray, fragmentProgram: ByteArray): void {
		...
		const vertexPrecision: string = (this._gl.getShaderPrecisionFormat(this._gl.VERTEX_SHADER, this._gl.HIGH_FLOAT).precision != 0) ? 'highp' : 'mediump';
		const fragmentPrecision: string = (this._gl.getShaderPrecisionFormat(this._gl.FRAGMENT_SHADER, this._gl.HIGH_FLOAT).precision != 0) ? 'highp' : 'mediump';

		const vertexString: string = ProgramGLES._aglslParser.parse(ProgramGLES._tokenizer.decribeAGALByteArray(vertexProgram), vertexPrecision);
		const fragmentString: string = ProgramGLES._aglslParser.parse(ProgramGLES._tokenizer.decribeAGALByteArray(fragmentProgram), fragmentPrecision);
		//(String.fromCharCode(OpCodes.uploadProgram)+""+this.id + "###"+vertexString +  "###" + fragmentString + "#END");

		const newSendbytes = new Byte32Array();
		...
		const localInt32View = new Int32Array(newSendbytes.byteLength / 4);
