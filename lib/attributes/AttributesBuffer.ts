...
export class AttributesBuffer extends AssetBase {
	...
	public cloneBufferView(): AttributesBuffer {
		const attributesBuffer: AttributesBuffer = new AttributesBuffer(this._stride, this._count);
	...
	public clone(): AttributesBuffer {
		...
		const len: number = this._viewVOs.length;
	...
	public _setAttributes(viewIndex: number, arrayBufferView: ArrayBufferView, offset: number = 0): void {
		...
		const vOffset: number = viewVO.offset;
		const vCount: number = array.length / vLength;
	...
	public _addView(view: AttributesView): void {
		const viewVO: ViewVO = new ViewVO(view);
		const len: number = this._viewVOs.length;
	...
	public _removeView(view: AttributesView): void {
		const viewIndex: number = view._index;
		let viewVO: ViewVO = this._viewVOs.splice(viewIndex, 1)[0];
		const len: number = this._viewVOs.length;

		viewVO.dispose();

		for (let i: number = viewIndex; i < len; i++) {
	...
	public _updateLength(): void {
		...
		const newLength: number = this._newStride * this._newCount;

		if (!this._buffer || this._buffer.byteLength != newLength) {
			const newBuffer: ArrayBuffer = new ArrayBuffer(newLength);
			const newView: Uint8Array = new Uint8Array(newBuffer, 0, newBuffer.byteLength);
			let viewVO: ViewVO;
			let vLength: number;
			let vOffset: number;
			let vOldOffset: number;
