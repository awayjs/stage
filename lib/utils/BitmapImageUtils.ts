...
export class BitmapImageUtils
{
    ...
    public static sampleBilinear(u:number, v:number, data:Uint8ClampedArray, width:number, height:number, texelSizeX:number = 1, texelSizeY:number = 1):number[] {
		var color00:number[] = BitmapImageUtils.sample(u, v, data, width, height);
		var color10:number[] = BitmapImageUtils.sample(u + texelSizeX, v, data, width, height);
		var color01:number[] = BitmapImageUtils.sample(u, v + texelSizeY, data, width, height);
		var color11:number[] = BitmapImageUtils.sample(u + texelSizeX, v + texelSizeY, data, width, height);

		var a:number = u;
		a = a - Math.floor(a);

		var interColor0:number[] = BitmapImageUtils.interpolateColor(color00, color10, a);
		var interColor1:number[] = BitmapImageUtils.interpolateColor(color01, color11, a);
	...
	public static sampleBox(x0:number, y0:number, x1:number, y1:number, data:Uint8ClampedArray, width:number, height:number):number[] {
		...
		for (y = fromY; y < toY; y++) {
			...
			for (x = fromX; x < toX; x++) {
				...
				var color:number[] = BitmapImageUtils.sample(x, y, data, width, height);
