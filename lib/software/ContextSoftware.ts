...
export class ContextSoftware implements IContextGL {
	...
	constructor(canvas: HTMLCanvasElement) {
		...
		const len: number = 100 * 100;
		const zbufferBytes: ArrayBuffer = new ArrayBuffer(len * 8);
	...
	public configureBackBuffer(width: number, height: number, antiAlias: number, enableDepthAndStencil: boolean): void {
		...
		const backBufferWidth = width * this._antialias;
		const backBufferHeight = height * this._antialias;

		//double buffer for fast clearing
		const len: number = backBufferWidth * backBufferHeight;
		const zbufferBytes: ArrayBuffer = new ArrayBuffer(len * 8);
		this._backBufferZ = new Float32Array(zbufferBytes, 0, len);
		this._backBufferZClear = new Float32Array(zbufferBytes, len * 4, len);
		for (let i: number = 0; i < len; i++)
	...
	private activateScreenMatrix(width: number, height: number, yflip: boolean): void {
		...
		const raw: Float32Array = this._screenMatrix._rawData;
	...
	public setRenderToTexture(target: TextureSoftware, enableDepthAndStencil: boolean, antiAlias: number, surfaceSelector: number, mipmapSelector: number): void {
		...
		let textureBufferColor: BitmapImage2D = this.textureBuffersColor[surfaceSelector];
		...
		let textureBufferZ: Float32Array = this.textureBuffersZ[surfaceSelector];
		let textureBufferZClear: Float32Array = this.textureBuffersZClear[surfaceSelector];
		if (textureBufferZ == null) {

			//double buffer for fast clearing
			const len: number = target.width * target.height;
			const zbufferBytes: ArrayBuffer = new ArrayBuffer(len * 8);
	...
	public drawIndices(mode: ContextGLDrawMode, indexBuffer: IndexBufferSoftware, firstIndex: number, numIndices: number): void {
		...
		const position0: Float32Array = new Float32Array(4);
		const position1: Float32Array = new Float32Array(4);
		const position2: Float32Array = new Float32Array(4);
		const position3: Float32Array = new Float32Array(4); // potentially interpolated
		const position4: Float32Array = new Float32Array(4); // potentially interpolated

		const varying0: Float32Array = new Float32Array(this._program.numVarying * 4);
		const varying1: Float32Array = new Float32Array(this._program.numVarying * 4);
		const varying2: Float32Array = new Float32Array(this._program.numVarying * 4);
		const varying3: Float32Array = new Float32Array(this._program.numVarying * 4); // potentially interpolated
		const varying4: Float32Array = new Float32Array(this._program.numVarying * 4); // potentially interpolated

		const incomingVertices: Array<Float32Array> = new Array<Float32Array>(5);
		const outgoingVertices: Array<Float32Array> = new Array<Float32Array>(4);
		...
		const incomingVaryings: Array<Float32Array> = new Array<Float32Array>(5);
		const outgoingVaryings: Array<Float32Array> = new Array<Float32Array>(4);
	...
	private _triangle(incomingVertices: Array<Float32Array>, outgoingVertices: Array<Float32Array>, incomingVaryings: Array<Float32Array>, outgoingVaryings: Array<Float32Array>): void {
		...
		let numOriginalVerticesInside: number = 0;
		let numInterpolations: number = 0;
		let numOutgoingVertices: number = 0;
		...
		let previousVertex: Float32Array = incomingVertices[2]; // p0
		let previousVarying: Float32Array = incomingVaryings[2]; // v0
		let previousVertexIsInside: boolean = previousVertex[2] > 0; // z > 0
		...
		for (let i: number = 0; i < 3; i++) {
			...
			const currentVertex: Float32Array = incomingVertices[i];
			const currentVarying: Float32Array = incomingVaryings[i];

			// Need interpolation?
			const currentVertexIsInside = currentVertex[2] > 0; // z > 0
			if (currentVertexIsInside != previousVertexIsInside) { // if one and only one is true
				...
				// z factor
				const dz_current: number = Math.abs(currentVertex[2]);
				const dz_previous: number = Math.abs(previousVertex[2]);
				const factorZ: number = dz_previous / (dz_previous + dz_current);
	...
	private _projectTriangle(position0: Float32Array, position1: Float32Array, position2: Float32Array, varying0: Float32Array, varying1: Float32Array, varying2: Float32Array): void {
		...
		// Skip invalid bounds.
		const w: number = this._bboxMax.x - this._bboxMin.x;
		const h: number = this._bboxMax.y - this._bboxMin.y;
		if (w <= 0 || h <= 0) { return; }

		// Calculate the area division for all barycentric coordinates.
		const area: number = (this._p1.x - this._p0.x) * (this._p2.y - this._p0.y) - (this._p1.y - this._p0.y) * (this._p2.x - this._p0.x); // simplified cross
		if (area < 0) { return; } // skip null areas.

		// Pre-calculate barycentric x and y derivatives (steps).
		const w0_dx: number = -(this._p2.y - this._p1.y) / area;
		const w1_dx: number = -(this._p0.y - this._p2.y) / area;
		const w2_dx: number = -(this._p1.y - this._p0.y) / area;
		const w0_dy: number = (this._p2.x - this._p1.x) / area;
		const w1_dy: number = (this._p0.x - this._p2.x) / area;
		const w2_dy: number = (this._p1.x - this._p0.x) / area;

		// Calculate top left barycentric coordinate.
		const w0: number = ((this._p2.x - this._p1.x) * (this._bboxMin.y - this._p1.y) - (this._p2.y - this._p1.y) * (this._bboxMin.x - this._p1.x)) / area; // simplified cross
		const w1: number = ((this._p0.x - this._p2.x) * (this._bboxMin.y - this._p2.y) - (this._p0.y - this._p2.y) * (this._bboxMin.x - this._p2.x)) / area; // simplified cross
		const w2: number = ((this._p1.x - this._p0.x) * (this._bboxMin.y - this._p0.y) - (this._p1.y - this._p0.y) * (this._bboxMin.x - this._p0.x)) / area; // simplified cross

		// Rasterize.
		let cx: number = 0;
		let cy: number = 0;
		for (let x: number = this._bboxMin.x; x <= this._bboxMax.x; x++) {
			for (let y: number = this._bboxMin.y; y <= this._bboxMax.y; y++) {
				...
				const index: number = (x % this._activeBufferColor.width) + y * this._activeBufferColor.width;
				const fragDepth: number = (this._barycentric.x * this._p0.z + this._barycentric.y * this._p1.z + this._barycentric.z * this._p2.z) / (this._barycentric.x + this._barycentric.y + this._barycentric.z);
				...
				const fragmentVO: ProgramVOSoftware = this._program.fragment(this, this._barycentric, this._barycentricRight, this._barycentricBottom, varying0, varying1, varying2, fragDepth);
	...
	public setSamplerStateAt(sampler: number, wrap: ContextGLWrapMode, filter: ContextGLTextureFilter, mipfilter: ContextGLMipFilter): void {
		let state: SoftwareSamplerState = this._samplerStates[sampler];
