...
export class ProgramSoftware implements IProgram {
	...
	public vertex(context: ContextSoftware, vertexIndex: number, position: Float32Array, varying: Float32Array): void {
		...
		var i: number;
		let j: number = 0;
		const numAttributes: number = this._vertexDescr.regread[0x0].length;
		const attributes: Float32Array = this._vertexVO.attributes;
		for (i = 0; i < numAttributes; i++) {
			const buffer: VertexBufferSoftware = context._vertexBuffers[i];

			if (!buffer)
				continue;

			const index: number = context._vertexBufferOffsets[i] / 4 + vertexIndex * buffer.attributesPerVertex;
		...
		//clear temps
		const temp: Float32Array = this._vertexVO.temp;
		const numTemp: number = temp.length;
		...
		const len: number = this._vertexDescr.tokens.length;
		for (var i: number = 0; i < len; i++) {
			const token: Token = this._vertexDescr.tokens[i];
	...
	public fragment(context: ContextSoftware, clip: Vector3D, clipRight: Vector3D, clipBottom: Vector3D, varying0: Float32Array, varying1: Float32Array, varying2: Float32Array, fragDepth: number): ProgramVOSoftware {
		...
		//clear temps
		const temp: Float32Array = this._fragmentVO.temp;
		const numTemp: number = temp.length;
		for (var i: number = 0; i < numTemp; i += 4) {
			temp[i] = 0;
			temp[i + 1] = 0;
		...
		const varyingDerivatives: number[] = [];
		const len: number = this._fragmentDescr.tokens.length;
		for (var i: number = 0; i < len; i++) {
		...
		const derivativeX: Float32Array = this._fragmentVO.derivativeX;
		const derivativeY: Float32Array = this._fragmentVO.derivativeY;

		const varying: Float32Array = this._fragmentVO.varying;
		const numVarying: number = varying.length;
	...
	public static mov(vo: ProgramVOSoftware, desc: Description, dest: Destination, source1: Destination, source2: Destination, context: ContextSoftware): void {
		const targetReg: number = 4 * dest.regnum;
		const source1Reg: number = 4 * source1.regnum;

		const target: Float32Array = ProgramSoftware.getDestTarget(vo, desc, dest);
		const source1Target: Float32Array = ProgramSoftware.getSourceTarget(vo, desc, source1, context);

		const mask: number = dest.mask;
	...
	public static m44(vo: ProgramVOSoftware, desc: Description, dest: Destination, source1: Destination, source2: Destination, context: ContextSoftware): void {
		const targetReg: number = 4 * dest.regnum;
		const source1Reg: number = 4 * source1.regnum;
		const source2Reg: number = 4 * source2.regnum;

		const target: Float32Array = ProgramSoftware.getDestTarget(vo, desc, dest);
		const source1Target: Float32Array = ProgramSoftware.getSourceTarget(vo, desc, source1, context);
		const source2Target: Float32Array = ProgramSoftware.getSourceTarget(vo, desc, source2, context);
	...
	private static sample(vo: ProgramVOSoftware, desc: Description, context: ContextSoftware, source1: Destination, textureIndex: number): Float32Array {
		const texture: ITextureBaseSoftware = context._textures[textureIndex];
	...
	private static sampleCubeTexture(vo: ProgramVOSoftware, desc: Description, context: ContextSoftware, source1: Destination, textureIndex: number): Float32Array {
		...
		const x: number = source1Target[source1Reg + ((source1.swizzle >> 0) & 3)];
		const y: number = source1Target[source1Reg + ((source1.swizzle >> 2) & 3)];
		const z: number = source1Target[source1Reg + ((source1.swizzle >> 4) & 3)];
		...
		const absmax: number = Math.max(absx, absy, absz);
		if (absmax == absx) {
			if (x >= 0) {
				side = 0;
				u = 0.5 - 0.5 * (z / x);
		...
		const texture: CubeTextureSoftware = <CubeTextureSoftware> context._textures[textureIndex];
		const state: SoftwareSamplerState = context._samplerStates[textureIndex] || this._defaultSamplerState;

		const repeat: boolean = state.wrap == ContextGLWrapMode.REPEAT;
		const mipmap: boolean = state.mipfilter == ContextGLMipFilter.MIPLINEAR;
	...
	private static sampleSimpleTexture(vo: ProgramVOSoftware, desc: Description, context: ContextSoftware, source1: Destination, textureIndex: number): Float32Array {
		...
		const u: number = source1Target[source1Reg + ((source1.swizzle >> 0) & 3)];
		const v: number = source1Target[source1Reg + ((source1.swizzle >> 2) & 3)];
		...
		const texture: TextureSoftware = <TextureSoftware> context._textures[textureIndex];
		const state: SoftwareSamplerState = context._samplerStates[textureIndex] || this._defaultSamplerState;

		const repeat: boolean = state.wrap == ContextGLWrapMode.REPEAT;
		const mipmap: boolean = state.mipfilter == ContextGLMipFilter.MIPLINEAR;
		if (mipmap && texture.getMipLevelsCount() > 1) {
			const dux: number = Math.abs(vo.derivativeX[source1Reg + ((source1.swizzle >> 0) & 3)]);
			const dvx: number = Math.abs(vo.derivativeX[source1Reg + ((source1.swizzle >> 2) & 3)]);
			const duy: number = Math.abs(vo.derivativeY[source1Reg + ((source1.swizzle >> 0) & 3)]);
			const dvy: number = Math.abs(vo.derivativeY[source1Reg + ((source1.swizzle >> 2) & 3)]);

			const lambda: number = Math.log(Math.max(texture.width * Math.sqrt(dux * dux + dvx * dvx), texture.height * Math.sqrt(duy * duy + dvy * dvy))) / Math.LN2;
			if (lambda > 0) {

				let miplevelLow: number = Math.floor(lambda);
				let miplevelHigh: number = Math.ceil(lambda);

				const maxmiplevel: number = Math.log(Math.min(texture.width, texture.height)) / Math.LN2;
				...
				const mipblend: number = lambda - Math.floor(lambda);

				const dataLow: Uint8ClampedArray = texture.getData(miplevelLow);
				const dataLowWidth: number = texture.width / Math.pow(2, miplevelLow);
				const dataLowHeight: number = texture.height / Math.pow(2, miplevelLow);
				const dataHigh: Uint8ClampedArray = texture.getData(miplevelHigh);
				const dataHighWidth: number = texture.width / Math.pow(2, miplevelHigh);
				const dataHighHeight: number = texture.height / Math.pow(2, miplevelHigh);
	...
	private static sampleNearest(u: number, v: number, textureData: Uint8ClampedArray, textureWidth: number, textureHeight: number, repeat: boolean, output: Float32Array): Float32Array {
		...
		const pos: number = (u + v * textureWidth) * 4;
	...
	private static sampleBilinear(u: number, v: number, textureData: Uint8ClampedArray, textureWidth: number, textureHeight: number, repeat: boolean, output: Float32Array): Float32Array {
		const texelSizeX: number = 1 / textureWidth;
		const texelSizeY: number = 1 / textureHeight;
		u -= texelSizeX / 2;
		v -= texelSizeY / 2;

		let a: number = u * textureWidth;
		a = a - Math.floor(a);

		let b: number = v * textureHeight;
		b = b - Math.floor(b);
	...
	public static tex(vo: ProgramVOSoftware, desc: Description, dest: Destination, source1: Destination, source2: Destination, context: ContextSoftware): void {
		...
		const color: Float32Array = ProgramSoftware.sample(vo, desc, context, source1, source2.regnum);
	...
	public static add(vo: ProgramVOSoftware, desc: Description, dest: Destination, source1: Destination, source2: Destination, context: ContextSoftware): void {
		...
		const mask: number = dest.mask;
		const source1Swizzle: number = source1.swizzle;
		const source2Swizzle: number = source2.swizzle;
	...
	public static nrm(vo: ProgramVOSoftware, desc: Description, dest: Destination, source1: Destination, source2: Destination, context: ContextSoftware): void {
		...
		let x: number = source1Target[source1Reg + ((source1Swizzle >> 0) & 3)];
		let y: number = source1Target[source1Reg + ((source1Swizzle >> 2) & 3)];
		let z: number = source1Target[source1Reg + ((source1Swizzle >> 4) & 3)];
	...
	public static crs(vo: ProgramVOSoftware, desc: Description, dest: Destination, source1: Destination, source2: Destination, context: ContextSoftware): void {
		...
		const source1Target: Float32Array = ProgramSoftware.getSourceTarget(vo, desc, source1, context);
		const source1TargetX: number = source1Target[source1Reg + ((source1Swizzle >> 0) & 3)];
		const source1TargetY: number = source1Target[source1Reg + ((source1Swizzle >> 2) & 3)];
		const source1TargetZ: number = source1Target[source1Reg + ((source1Swizzle >> 4) & 3)];

		const source2Target: Float32Array = ProgramSoftware.getSourceTarget(vo, desc, source2, context);
		const source2TargetX: number = source2Target[source2Reg + ((source2Swizzle >> 0) & 3)];
		const source2TargetY: number = source2Target[source2Reg + ((source2Swizzle >> 2) & 3)];
		const source2TargetZ: number = source2Target[source2Reg + ((source2Swizzle >> 4) & 3)];
	...
	public static dp4(vo: ProgramVOSoftware, desc: Description, dest: Destination, source1: Destination, source2: Destination, context: ContextSoftware): void {
		...
		const source1TargetY: number = source1Target[source1Reg + ((source1Swizzle >> 2) & 3)];
		const source1TargetZ: number = source1Target[source1Reg + ((source1Swizzle >> 4) & 3)];
		const source1TargetW: number = source1Target[source1Reg + ((source1Swizzle >> 6) & 3)];

		const source2Target: Float32Array = ProgramSoftware.getSourceTarget(vo, desc, source2, context);
		const source2TargetX: number = source2Target[source2Reg + ((source2Swizzle >> 0) & 3)];
		const source2TargetY: number = source2Target[source2Reg + ((source2Swizzle >> 2) & 3)];
		const source2TargetZ: number = source2Target[source2Reg + ((source2Swizzle >> 4) & 3)];
		const source2TargetW: number = source2Target[source2Reg + ((source2Swizzle >> 6) & 3)];
	...
	public static ddx(vo: ProgramVOSoftware, desc: Description, dest: Destination, source1: Destination, source2: Destination, context: ContextSoftware): void {
		...
		const target: Float32Array = ProgramSoftware.getDestTarget(vo, desc, dest);
		const source1Target: Float32Array = vo.derivativeX;
	...
	public static ddy(vo: ProgramVOSoftware, desc: Description, dest: Destination, source1: Destination, source2: Destination, context: ContextSoftware): void {
		...
		const target: Float32Array = ProgramSoftware.getDestTarget(vo, desc, dest);
		const source1Target: Float32Array = vo.derivativeY;
	...
	public static seq(vo: ProgramVOSoftware, desc: Description, dest: Destination, source1: Destination, source2: Destination, context: ContextSoftware): void {
		const targetReg: number = 4 * dest.regnum;
		const source1Reg: number = 4 * source1.regnum;
		const source2Reg: number = 4 * source1.regnum;
