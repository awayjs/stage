...
export class Filter3DThresholdTask extends Filter3DTaskBase
{
	...
	public set color(value:number)
	{
		...
		//pre-multiply alpha
		var alpha:number = ((value >> 24) & 0xff)/0xff;
	...
	public getFragmentCode():string
	{
		...
		var threshold:ShaderRegisterElement = this._registerCache.getFreeFragmentConstant();
		this._fragmentConstantsIndex = threshold.index;

		var color:ShaderRegisterElement = this._registerCache.getFreeFragmentConstant();

		var mask:ShaderRegisterElement = this._registerCache.getFreeFragmentConstant();

		var decode:ShaderRegisterElement = this._registerCache.getFreeFragmentConstant();
	...
	public activate(stage:Stage, projection:ProjectionBase, depthTexture:Image2D):void
	{
		...
		var data:Float32Array = this._vertexConstantData;
		...
		var tVector:Vector3D = new Vector3D(this._thresholdRGBA.x*this._maskRGBA.x, this._thresholdRGBA.y*this._maskRGBA.y, this._thresholdRGBA.z*this._maskRGBA.z, this._thresholdRGBA.w*this._maskRGBA.w);
		var tValue:number = tVector.dotProduct(this._decodeVector);
