...
export class Filter3DCompositeTask extends Filter3DTaskBase {
	...
	public getFragmentCode(): string {
		...
		const temp4: ShaderRegisterElement = this._registerCache.getFreeFragmentVectorTemp();
		...
		const exposure: ShaderRegisterElement = this._registerCache.getFreeFragmentConstant();
		this._exposureIndex = exposure.index * 4;

		const scaling: ShaderRegisterElement = this._registerCache.getFreeFragmentConstant();
	...
	public activate(stage: Stage, projection: ProjectionBase, depthTexture: Image2D): void {
		...
		this._overlayTexture
			.getAbstraction<_Stage_ImageBase>(stage)
			.activate(this._overlayTextureIndex);
	}
