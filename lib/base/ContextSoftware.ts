...
class ContextSoftware implements IContextGL {
    ...
    public setProgramConstantsFromArray(programType:string, firstRegister:number, data:number[], numRegisters:number) {
        ...
        if (firstRegister == 0 && numRegisters == 4) {
            ...
            //view.appendScale(0.5, 0.5,1);
            //view.appendTranslation(this._backBufferWidth*0.5, this._backBufferHeight*0.5,0);
            //this._projectionMatrix.prepend(view);
    ...
    public triangle(p0:Vector3D, p1:Vector3D, p2:Vector3D, uv1:Point, uv2:Point, uv3:Point):void {
        p0.scaleBy(1/p0.w);
        p1.scaleBy(1/p1.w);
        p2.scaleBy(1/p2.w);
